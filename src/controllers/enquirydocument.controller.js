...
const enquiryDocumentModel = require("../models/enquirydocument.model");
const fs = require("fs");
...
// ************************* Upload Document *******************************
exports.uploadEnquiryDocument = async (req, res, next) => {
  ...
  try {
    ...
    if (!req.file) {
      ...
      return res
        .status(400)
        .send({ message: "Please select a correct file format to upload!" });
    }
    logger.info("Requested Payload Data:", data);
    const result = await enquiryDocumentModel.createEnquiryDocumentQuery(data);
    ...
    res.send({
      message: error.message || "Some Error Ocurred while uploading Document",
    });
  }
};
...
// ************************ get Documnet By Enquiry Id ******************************
exports.getAllEnquiryDocumentByEnquiryId = async (req, res, next) => {
  ...
  try {
    ...
    const documentlist =
      await enquiryDocumentModel.getAllEnquiryDocumentByEnquireIdQuery(
    ...
    if (!documentlist) {
      ...
      documentlist.map((item) => {
        item.filePath = findFilePathByFileName(
          item.FileName,
          req.params.enquiryid
        );
      });
    ...
    logger.error(error.message);
    res.status(500).send({
      message: error.message || "Some Error Ocurred while Getting All Document",
    });
  }
};

// ************************** Get Document by Id *****************************
exports.getEnquiryDocumentById = async (req, res, next) => {
  ...
  try {
    ...
    const document = await enquiryDocumentModel.getEnquiryDocumentbyIdQuery(
      req.params.id
    );
    if (document && document.length == 0) {
      ...
      return res
        .status(400)
        .send({ message: "Document not found for this Id" });
    }
    next();
  } catch (error) {
    logger.error(error.message);
    res.status(500).send({
      message: error.message || "Some Error Ocurred while Getting Document",
    });
  }
};

// ************************* Documnet Update *******************************
exports.updateEnquiryDocumentById = async (req, res, next) => {
  ...
  try {
    ...
    const result = await enquiryDocumentModel.updateEnquiryDocumentQuery(
      req.file.filename,
      req.body.EditedBy,
      new Date(Date.now()),
      req.params.id
    ...
    if (!result) {
      ...
      const filepath = await findFilePathByFileName(
        req.body.FileName,
        req.body.EnquiryId
      );
      ...
      if (filepath) {
        fs.unlink(`./${filepath}`, (error) => {
          ...
          logger.success(
            "Document Updated Successfully with deleted existing file from DIR:",
            `./${filepath}`
          );
          res.status(200).send({
            status: 200,
    ...
    logger.error(error.message);
    res.status(500).send({
      message: error.message || "Some Error Ocurred while Update Document",
    });
  }
};

// ********************** document delete *******************************
exports.deletEnquiryDocument = async (req, res, next) => {
  try {
    logger.info("Document Id to deleted", req.params);
    if (!req.params.id || req.params.id === "" || req.params.id === ":id") {
      logger.error("id can not be empty!");
      return res.status(400).send({ message: "id can not be empty!" });
    }

    const result = await enquiryDocumentModel.deleteEnquiryDocumentQuery(
      req.params.id
    );

    if (!result) {
      logger.error("Unable to delete Document");
      ...
      if (filepath) {
        fs.unlink(`./${filepath}`, (error) => {
          ...
          logger.success(
            "Document Deleted Successfully with deleted existing file from DIR:",
            `./${filepath}`
          );
          res.status(200).send({
            status: 200,
    ...
    res.status(500).send({
      message:
        error.message || "Some Error Ocurred while Deleteing the Document",
    });
  }
};

// ************************* document download ************************************
exports.downloadEnquiryDocumentByPath = async (req, res, next) => {
  try {
    ...
    logger.error(error);
    res.status(500).send({
      message: error.message || "Some Error Ocurred while Download Document",
    });
  }
};

// ******************************* View Documnet ***********************************
exports.viewEnquiryDocument = async (req, res, next) => {
  try {
    logger.info("Requested Params:", req.params);
    ...
    logger.error(error);
    res.status(500).send({
      message: error.message || "Some Error Ocurred while View Document",
    });
  }
};
