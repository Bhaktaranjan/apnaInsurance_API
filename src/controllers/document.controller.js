...
exports.getAllDocumentByEnquiryId = async (req, res, next) => {
  ...
  try {
    ...
    const documentlist = await documentmodel.getAlldocumentByEnquireId(
...
// ********************** document delete *******************************
exports.deletDocument = async (req, res, next) => {
  try {
    ...
    const result = await documentmodel.deleteDocument(req.params.id);
    ...
    logger.error(error.message);
    res.status(500).send({
      message: error.message || "Some Error Ocurred while Deleteing the Document",
    });
  }
};
