...
const upload = multer({
  ...
  fileFilter(req, file, cb, err) {
    ...
    if (Array.isArray(JSON.parse(process.env.ALLOWED_FILE_MIMETYPES))) {
      for (
        let index = 0;
        index < JSON.parse(process.env.ALLOWED_FILE_MIMETYPES).length;
        index++
      ) {
        const element = JSON.parse(process.env.ALLOWED_FILE_MIMETYPES)[index];
        allowedFileMimetypes.push(element.mimetype);
      }
      console.log("allowedFileMimetypes", allowedFileMimetypes);

      if (
        allowedFileMimetypes &&
        allowedFileMimetypes.includes(file.mimetype.toLowerCase())
      ) {
        req.file = file;
        cb(null, true);
      } else {
